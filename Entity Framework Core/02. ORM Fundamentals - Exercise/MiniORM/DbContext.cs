namespace MiniORM
{
    ...
    public abstract class DbContext
    {
        ...
        private void PopulateDbSet<TEntity>(PropertyInfo dbSet)
            where TEntity : class, new()
        {
            IEnumerable<TEntity> entities = LoadTableEntoties<TEntity>();
        ...
        private IEnumerable<TEntity> LoadTableEntoties<TEntity>()
            where TEntity : class
        {
            Type table = typeof(TEntity);

            string[] columns = GetEntityColumnName(table);
        ...
        private string[] GetEntityColumnName(Type table)
        {
            ...
            var dbColumn = this.connection.FetchColumnNames(tableName);

            string[] columns = table.GetProperties()
                .Where(pi => dbColumn.Contains(pi.Name) &&
