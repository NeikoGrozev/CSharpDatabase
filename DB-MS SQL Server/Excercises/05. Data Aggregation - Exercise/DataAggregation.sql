...
SELECT [Ages].[Age Group], COUNT([Ages].[Age Group]) AS WizardCount 
FROM (
	SELECT 
		CASE
...
FROM Employees AS e
WHERE Salary > (SELECT AVG(Salary) FROM Employees AS Emplo WHERE Emplo.DepartmentID = e.DepartmentID)